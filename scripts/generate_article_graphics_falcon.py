...
OUT_DIR = Path(os.environ.get("ARTICLE_GRAPHICS_DIR", ROOT / "figures" / "article_falcon"))
...
ALGORITHM_SHORT = {
    "Falcon-Precomputed-512": "Falcon-Precomputed-512",
...
LABEL_BORDER = "#D7DEE8"
VALUE_SIZE = 34
STRESS_VALUE_SIZE = 30
TICK_SIZE = 35
AXIS_SIZE = 38
LEGEND_TITLE_SIZE = 38
LEGEND_SIZE = 35
CATEGORY_SIZE = 34
CATEGORY_LINE_HEIGHT = 44
...
def draw_png_multiline(
    ...
    y: float,
    lines: list[str],
    size: int = CATEGORY_SIZE,
    fill: str = "#425466",
    anchor: str = "ma",
    line_height: int = CATEGORY_LINE_HEIGHT,
) -> None:
...
def label_text_dims(text: str, size: int) -> tuple[float, float]:
...
def clamp_label_center(
    ...
    tw, th = label_text_dims(text, size)
...
def linear_ticks(values: list[float], count: int = 5) -> tuple[list[float], float, float]:
    ...
    step = nice_step(padded_max / count)
...
def render_isolated_metric_png(
    ...
    if compact_bars:
        ...
        for idx, (alg, value) in enumerate(values):
            x = left if idx == 0 else right - bar_w
            cx = x + bar_w / 2
            color = COLORS[alg]
            y = y_map(value)
    ...
    for idx, (alg, value) in enumerate(values):
        ...
        y = y_map(value)
        if log_scale:
            draw.line((cx, baseline, cx, y), fill=color, width=20)
            draw.ellipse(
...
def render_isolated_resource_png(
    name: str,
    y_label: str,
    values: list[tuple[str, float]],
) -> Path:
    ...
    y_map, ticks, _, _ = make_y_map(raw_values, False)
    draw_png_axes(img, draw, y_label, ticks, y_map)
...
def render_stress_metric_png(
    ...
    def x_map(v: int) -> float:
        if x_max == x_min:
            return (left + right) / 2
        return left + x_pad + (v - x_min) / (x_max - x_min) * (right - left - 2 * x_pad)
    ...
    for alg in ALGORITHM_ORDER:
        ...
        draw.line([(x, y) for x, y, _ in points], fill=color, width=9, joint="curve")
...
def main() -> None:
    ...
    for name, fig_id, title, y_label, accessor, log_scale, metric in isolated_specs:
        ...
        if metric in {"cpu_utilization_avg_pct", "memory_alloc_avg_mb"}:
            render_isolated_resource_png(name, y_label, values)
        else:
